...
/*
   ...
   javac -cp activation.jar:javax.mail.jar: sendAttachment.java
   The program is run with: 
   java -cp activation.jar:javax.mail.jar: sendAttachment thisfile.txt
*/
...
*/
      ...
      try {
         ...
         // The message due to the attachment has to be made into a multi part 
         // Rather than standard text
         Multipart multipart = new MimeMultipart();
         // Allocate a variable for the body
         BodyPart messageBodyPart = new MimeBodyPart();
         // Set the text for the body
         messageBodyPart.setText(parsedOutput[7]);
         // Add the body to the main multi part object
         multipart.addBodyPart(messageBodyPart);
         // Create another body part for the attachment
         messageBodyPart = new MimeBodyPart();
         // Create a string that contains the path, change depending on the file name
         String filename = "./water.jpg";
         // Create a datasource variable to contain the bytes
         DataSource source = new FileDataSource(filename);
         // Set the datahandler of the message to the datasource variable
         messageBodyPart.setDataHandler(new DataHandler(source));
         // Set the name to find the path when reading from file
         messageBodyPart.setFileName(filename);
         // Attach the attachment body to the main multipart object
         multipart.addBodyPart(messageBodyPart);
